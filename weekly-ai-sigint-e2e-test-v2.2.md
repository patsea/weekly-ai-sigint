# Weekly AI Sigint — Full End-to-End Test (v2.2)

**Created**: 2026-01-02
**Purpose**: Complete verification with fixed greenlet errors and Python-based credential validation
...
grep "^ANTHROPIC_API_KEY=" .env 2>/dev/null | cut -d= -f2 | tr -d '"' | tr -d "'" | python3 -c "
import sys
key = sys.stdin.read().strip()
length = len(key)
if length > 100 and key.startswith('sk-ant-api03-'):
    print('STATUS: VALID')
    print('Length: ' + str(length) + ' characters')
    print('Prefix: ' + key[:15] + '...')
elif length > 100:
    print('STATUS: INVALID (wrong prefix)')
    print('Length: ' + str(length) + ' characters')
    print('Expected prefix: sk-ant-api03-')
    print('Actual prefix: ' + key[:15])
else:
    print('STATUS: MISSING or TOO SHORT')
    print('Length: ' + str(length) + ' characters (need 100+)')
"
```
...
grep "^NOTION_TOKEN=" .env 2>/dev/null | cut -d= -f2 | tr -d '"' | tr -d "'" | python3 -c "
import sys
...
if length > 40:
    print('STATUS: VALID')
    print('Length: ' + str(length) + ' characters')
    print('Prefix: ' + token[:10] + '...')
else:
    print('STATUS: MISSING or TOO SHORT')
    print('Length: ' + str(length) + ' characters (need 40+)')
"
```
...
grep "^NOTION_PAGE_ID=" .env 2>/dev/null | cut -d= -f2 | tr -d '"' | tr -d "'" | python3 -c "
import sys
page_id = sys.stdin.read().strip()
length = len(page_id)
if length >= 32:
    print('STATUS: VALID')
    ...
    print('STATUS: MISSING or TOO SHORT')
    print('Length: ' + str(length) + ' characters (need 32+)')
"
```
...
grep "^SLACK_WEBHOOK_URL=" .env 2>/dev/null | cut -d= -f2 | tr -d '"' | tr -d "'" | python3 -c "
import sys
url = sys.stdin.read().strip()
length = len(url)
if length > 70 and url.startswith('https://hooks.slack.com/services/'):
    print('STATUS: VALID')
    print('Length: ' + str(length) + ' characters')
    print('URL prefix: ' + url[:40] + '...')
elif length > 70:
    print('STATUS: INVALID (wrong format)')
    print('Expected: https://hooks.slack.com/services/...')
    print('Got: ' + url[:40] + '...')
else:
    print('STATUS: MISSING or TOO SHORT')
    print('Length: ' + str(length) + ' characters (need 70+)')
"
```
...
anth = check_env('ANTHROPIC_API_KEY', 100)
notion = check_env('NOTION_TOKEN', 40)
page = check_env('NOTION_PAGE_ID', 32)
slack = check_env('SLACK_WEBHOOK_URL', 70)

print('✅ ANTHROPIC_API_KEY: Configured' if anth else '❌ ANTHROPIC_API_KEY: MISSING (tests will fail)')
print('✅ NOTION_TOKEN: Configured' if notion else '⏭️  NOTION_TOKEN: Skipped')
print('✅ NOTION_PAGE_ID: Configured' if page else '⏭️  NOTION_PAGE_ID: Skipped')
print('✅ SLACK_WEBHOOK_URL: Configured' if slack else '⏭️  SLACK_WEBHOOK_URL: Skipped')
"
echo ""
```
